...
def one_to_one_clean_ER(dfpathA, dfpathB,n_epochs=100,p=20, q=1, n_similar=10, n_top=10, embedding_size=128, walk_length=10,context_size=10, use_faiss=True,file_directory=None, load_embedding_file=False, load_graph=False, load_n_best=False):
    ...
    """
    matches = entity_resolution(dfpathA, dfpathB, file_directory=None,p=p, q=q, n_epochs=n_epochs, n_similar=n_similar, n_top=n_top, embedding_size=embedding_size, walk_length=walk_length,context_size=context_size, use_faiss=use_faiss, load_embedding_file=load_embedding_file, load_graph=load_graph, load_n_best=load_n_best)
    return matches


def free_entity_resolution(df_list,file_directory=None,n_epochs=100,p=20, q=1, n_similar=10, n_top=10, embedding_size=128, walk_length=10,context_size=10, load_embedding_file=False, load_graph=False):
    """
        Params:
    ...
    # for i in range(len(n_top_pairs)):
    #     for j in range(len(n_top_pairs[i])):
    #         if n_top_pairs[i][j] <= i:
    #             n_top_pairs[i][j] = -1
